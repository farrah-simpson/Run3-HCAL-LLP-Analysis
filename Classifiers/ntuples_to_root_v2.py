    ...
    with open(input_text_file, "r") as f: 
        for root_file in f:
            ...
            if match_1 and match_2:  # Only proceed if both patterns are found
                ...
                command = [
                    "root", "-q", "-b", "-l",
                    f'NTupleEventToJetTree2_v2.C("{root_file}", "temp_signal/jetntuple_ggH_HToSSTobbbb_{m_tag}_{out_tag}.root") '
                ]
...
def NTuples_to_Root_bkgfiles(input_text_file):
    with open(input_text_file, "r") as f: 
        exo_tag = r"\/([A-Za-z0-9\-]+-EXOLLPJetHCAL-PromptReco)"
        ...
        for root_file in f:
            ...
            command = [
                "root", "-q", "-b", "-l",
                f'NTupleEventToJetTree2_v2.C("{root_file}", "temp_bkg/jets_{x_tag}_{out_tag}.root")'
            ]
...
if __name__ == "__main__":
    input_text_file = "../Run/InputFiles_2023_11_29_ggH_HTooSSTobbbb_MHH-350_MS-80_CTau500_ntuplesv3.txt"
    if "Run2023" in input_text_file: NTuples_to_Root_bkgfiles(input_text_file)
    if "ggH" in input_text_file: NTuples_to_Root_sigfiles(input_text_file)
    
    #output_file = "../Train/Inputs/hadded/HADD_Run2023D-EXOLLPJetHCAL-PromptReco-v2.root"
    #output_file = "Inputs/HADD_Run2023C-EXOLLPJetHCAL-PromptReco-v1.root"
    #input_dir = "Inputs/"+input_text_file+"/"
